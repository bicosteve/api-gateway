...
public record EventCacheProjection(
        @JsonProperty("schema_version") int schemaVersion, Long id,
        ...
        public static EventCacheProjection from(Event event) {
            return new EventCacheProjection(1, event.getId(), event.getEventId(), event.getEventUuid(), event.getSportId(),
                    event.getEventDate(), event.getSeasonType(), event.getSeasonYear(), event.getEventName(),
                    event.getEventHeadline(), event.getEventStatus(),
        ...
        public Event toEvent() {
            Event event = Event.builder().id(id).eventId(eventId).eventUuid(eventUuid).sportId(sportId)
                    .eventDate(eventDate).seasonType(seasonType).seasonYear(seasonYear).eventName(eventName)
                    .eventHeadline(eventHeadline).eventStatus(eventStatus).build();
        ...
        record TeamProjection(Long id, @JsonProperty("team_id") Long teamId, @JsonProperty("event_id") String eventId,
                              String name, String abbreviation, @JsonProperty("is_home") boolean isHome,
                              @JsonProperty("is_away") boolean isAway, @JsonProperty("league_name") String leagueName) {
            static TeamProjection from(Team team) {
                return new TeamProjection(team.getId(), team.getTeamId(), team.getEventId(), team.getName(),
                        team.getAbbreviation(), team.getIsHome() != 0, team.getIsAway() != 0, team.getLeagueName());
            }
            Team toModel() {
                return Team.builder().id(id).teamId(teamId).eventId(eventId).name(name).abbreviation(abbreviation)
                        .isHome(isHome ? 1 : 0).isAway(isAway ? 1 : 0).leagueName(leagueName).build();
            }
        }

        record MarketProjection(@JsonProperty("local_id") Long localId, @JsonProperty("market_id") Integer marketId,
                                @JsonProperty("market_type_id") Integer marketTypeId, @JsonProperty("period_id") int periodId,
                                String name, @JsonProperty("market_description") String marketDescription,
                                @JsonProperty("event_id") String eventId, List<ParticipantProjection> participants) {
            static MarketProjection from(Market market) {
                return new MarketProjection(market.getId(), market.getMarketRundownId(), market.getMarketTypeId(),
                        market.getPeriodId() == null ? 0 : market.getPeriodId(), market.getName(), market.getDescription(),
                        market.getEventId(), market.getParticipants() == null ? List.of()
                        : market.getParticipants().stream().map(ParticipantProjection::from).toList());
            }
            Market toModel() {
                return Market.builder().id(localId).marketRundownId(marketId).marketTypeId(marketTypeId).periodId(periodId)
                        .name(name).description(marketDescription).eventId(eventId).participants(participants == null ? List.of()
                        : participants.stream().map(ParticipantProjection::toModel).toList()).build();
            }
        }

        record ParticipantProjection(@JsonProperty("participant_id") Long participantId, Long id, String type, String name,
                                     @JsonProperty("market_id") Integer marketId, List<LineProjection> lines) {
            static ParticipantProjection from(Participant participant) {
                ...
                return new ParticipantProjection(participant.getParticipantId(), (long) participant.getRundownId(), participant.getType(),
                        participant.getName(), participant.getMarketId() == null ? null : participant.getMarketId().intValue(),
        ...
        record PriceProjection(@JsonProperty("price_id") Integer priceId, @JsonProperty("rundown_id") String rundownId,
                               Integer price, @JsonProperty("is_main_line") boolean isMainLine, BigDecimal odds,
                               @JsonProperty("participant_id") Long participantId, @JsonProperty("handicap_value") String handicapValue,
                               @JsonProperty("line_id") String lineId, @JsonProperty("closed_at") OffsetDateTime closedAt) {
        ...
        record ScoreProjection(@JsonProperty("score_id") Long scoreId, @JsonProperty("event_id") String eventId,
                               @JsonProperty("event_status") Integer eventStatus,
                               @JsonProperty("event_status_detail") String eventStatusDetail,
                               @JsonProperty("team_id_away") Integer teamIdAway, @JsonProperty("team_id_home") Integer teamIdHome,
                               @JsonProperty("winner_away") Integer winnerAway, @JsonProperty("winner_home") Integer winnerHome,
                               @JsonProperty("score_away") Integer scoreAway, @JsonProperty("score_home") Integer scoreHome,
                               @JsonProperty("game_clock") Integer gameClock, @JsonProperty("game_period") Integer gamePeriod) {
